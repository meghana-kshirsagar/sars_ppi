...
if __name__ == "__main__":
    ...
    if True:
        ...
        for split in range(0,5):
            ...
            curr_perf = []
            model_file = format("%s/split%d_1to%d_int%d_trial%d.pkl" % (out_dir, split, int(negfrac), interac, trial))
            clf = pickle.load(open(model_file, 'rb'))
            y_pred_cov = clf.predict(X_cov)
            #curr_perf += [metrics.accuracy_score(y_cov, y_pred_cov)]
            print(metrics.confusion_matrix(y_cov, y_pred_cov))
            y_pred_cov = clf.predict_proba(X_cov)
            curr_perf += [get_aucpr_R(y_cov, y_pred_cov[:,1])]
            curr_perf += [get_auc_R(y_cov, y_pred_cov[:,1])]
            curr_perf += [get_fmax(y_cov, y_pred_cov[:,1])]
            curr_perf += get_early_prec(y_cov, y_pred_cov[:,1])
            print(curr_perf)
